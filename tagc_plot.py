...
max_phylum_plot = int(sys.argv[2])
fig_format = 'png'
almost_black = '#262626'
...
for phylum in sorted_phyla:
	if i < max_phylum_plot:
		...
		if phylum != 'Not annotated':
			...
			axHistx.hist(x_i,  color = hex_color, bins=top_bins)
			right_bins = np.logspace(-1, 10, 500, base=10.0)
			axHisty.hist(y_i, color = hex_color, bins=right_bins, histtype='bar', orientation='horizontal')
			...
			top_bins = np.arange(0, 1, 0.01)
			axHistx.hist(x_i,  color = '#d3d3d3', bins=top_bins)
			right_bins = np.logspace(-1, 10, 500, base=10.0)
			axHisty.hist(y_i, color = '#d3d3d3', bins=right_bins, histtype='bar', orientation='horizontal')
		...
		plt.savefig(blob_file + '_hist_' + str(i) + '.png', format=fig_format)
		print " [Done]"
	else:
		break
...
plt.savefig(blob_file + '_hist.png', format=fig_format)
print " [Done]"
#plt.show()